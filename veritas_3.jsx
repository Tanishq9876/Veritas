...
async function analyseText(text) {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      system: SYSTEM,
      messages: [{ role: "user", content: `Fact-check the following:\n\n${text}` }],
    }),
  });
...
async function analyseImage(base64, mimeType, caption = "") {
  const userContent = [
    ...
    { type: "text",  text: `Please fact-check this image.${caption ? ` The user added this caption: "${caption}"` : ""} Describe what you see, extract any visible text or claims, and evaluate their accuracy.` },
  ];
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
          ...
          <div style={{ animation: "fade 0.3s ease" }}>
            ...
            {!result && (
              <div style={{ marginBottom: 36, maxWidth: 600 }}>
                <h1 style={{ fontFamily: T.fontDisplay, fontSize: "clamp(30px, 5vw, 44px)", fontWeight: 700, lineHeight: 1.15, letterSpacing: "-0.03em", marginBottom: 14 }}>
